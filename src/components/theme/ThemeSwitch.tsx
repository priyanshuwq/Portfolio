...
export const useThemeToggle = ({
  ...
  const toggleTheme = useCallback((event?: React.MouseEvent<HTMLButtonElement>) => {
    ...
    // Get click position for dynamic animation origin
    let clickX = 50;
    let clickY = 50;
    ...
    if (event && typeof window !== 'undefined') {
      ...
      clickX = (buttonCenterX / window.innerWidth) * 100;
      clickY = (buttonCenterY / window.innerHeight) * 100;
    }

    const animation = createAnimation(variant, start, blur, gifUrl, clickX, clickY);
  ...
  const setCrazyLightTheme = useCallback((event?: React.MouseEvent<HTMLButtonElement>) => {
    setIsDark(false);
  ...
  const setCrazyDarkTheme = useCallback((event?: React.MouseEvent<HTMLButtonElement>) => {
    setIsDark(true);
  ...
  if (variant === 'rectangle') {
    ...
    return {
      ...
      @keyframes reveal-dark-${start}${blur ? '-blur' : ''} {
        ...
        ${blur ? '30% { filter: blur(4px); }' : ''}
        ${blur ? '60% { filter: blur(2px); }' : ''}
        ${blur ? '85% { filter: blur(0.5px); }' : ''}
        to {
